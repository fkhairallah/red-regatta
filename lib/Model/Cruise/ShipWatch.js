...
class ShipWatch {
    ...
    loadFromObject(o) {
        ...
        o.reservedList.forEach((element) => {
     ...
     */
    setupWatch() {
        let wStart = this.startDate.getHours() % this.hoursInWatch;
        this.watchList = [];
    ...
    getWatchArchetype(date) {
        ...
        if (result.length != 1)
            console.error("Filter ERROR", hour, result);
        return result[0];
    }
    /**
     ...
     */
    getTodaysWatches() {
        let date = (0, date_fns_1.subHours)(new Date(), 2 * this.hoursInWatch);
