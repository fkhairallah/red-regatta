...
/*[Bindable]*/ class NMEA extends events_1.EventEmitter {
    constructor(frequencyOfSamplingInSeconds = 1, useGPSClock = false) {
        ...
        // $xxRMC sentences
        this.useGPSClock = false; // if set, GPS time is used to issue sailpoint events
        this.sentenceCounter = 0; // counts sentences
        this.badSentenceCount = 0; // number of malformed sentences
    ...
    */
    parseGGA(tokens) {
        try {
            var x = NMEA.latlon2Decimal(tokens[2], tokens[3]);
            if (isNaN(x))
    ...
    genGGA(sp) {
        var s = "$RRGGA,{this.genNMEATime(sp.timeStamp)},{A},{this.decimalToNMEAAngle(sp.lat)},{this.genNorthSouth(sp.lat)},{this.decimalToNMEAAngle(sp.lon)},{this.genEastWest(sp.lon)},{sp.speedOverGround.toFixed(1)},{Math.trunc(sp.courseOverGround)},{this.genNMEADate(sp.timeStamp)},{Math.abs(this.magneticDeclination).toFixed(1)},{this.genEastWest(this.magneticDeclination)},{A}*";
        s += NMEA.calculateCheckSum(s);
        return s;
    }
    ...
    *
     ...
     *
     * ******************************************************************************************/
    parseNMEATimeAndDate(nmeaTime, nmeaDate) {
        var yy;
        var mm;
        var dd;
        var t = new Date();
...
exports.NMEA = NMEA;
NMEA.NMEA_EVENT_GPS_DATA = "NMEA_EVENT_GPS_DATA";
NMEA.NMEA_EVENT_NEW_POINT = "NMEA_EVENT_NEW_POiNT";
