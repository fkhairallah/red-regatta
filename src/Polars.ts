...
public myPolars:Array<PolarTarget>;
...
public loadFromObject(o:Polars):void
{
	try
	{
		if (o != null)
 ...
 * **************************************************************************/
public loadFromCSV(theName:string, rawData:string):void
{
	this.name = theName;
	
	var pol:string[] = rawData.split("\r\n");
	
	this.myPolars = new Array;
	
	pol.forEach ( (tws:string) =>
	{
		var polTarget:any[] = tws.split(",");
		if ( (polTarget.length > 8) && !isNaN(polTarget[0]) )
		{
			var pt:PolarTarget = new PolarTarget;
...
{
	var exp:string = "TWS,TWA,AWS,AWA,V,VMG,HEEL\r\n";
	
	this.myPolars.forEach ( (pt:PolarTarget) => {
 ...
 * **************************************************************************/
public loadFromCPN(theName:string, rawData:string):void
{
	this.name = theName;
	
	var pol:any[] = rawData.split("\r\n");
	
	this.myPolars = new Array();
	
	// parse first line and extract wind speed
	var tws:any[] = pol[0].split(";");
	
	for (var i:number=1;i<(pol.length-1);i++)
	{
		var polTarget:any[] = pol[i].split(";");
		if ( !isNaN(polTarget[0]) )
		{
			for (var j:number=1;j<polTarget.length;j++)
			{
				var pt:PolarTarget = new PolarTarget();
...
public saveToCPN():string
{
	var exp:string = "twa/tws";
	var twa:Array<number> = new Array;
	var tws:Array<number> = new Array;
	var thisV:number;
	...
	{
		if ( (tws.length == 0) ||
			(pt.tws > tws[tws.length-1]) ) tws.push(pt.tws); 
	})

	tws.forEach ( (i:number) => 
	{
		exp += ";" + i.toString();
	})
	exp += "\r\n";
	...
	this.myPolars.forEach ( (pt:PolarTarget ) =>
	{
		if (!twa.find( t => t === pt.twa)) twa.push(pt.twa);
	} );
	twa.sort();

	
	// now output twa one line at a time
	twa.forEach( (thisTWA:number) =>
	{
		exp += thisTWA.toString();
		
		tws.forEach ( (thisTWS:number) =>	// each speed entry is seperated by tab
		{
			var target:PolarTarget = this.getTarget(thisTWS, thisTWA);
 ...
 * 
 * *************************************************************************/
public loadFromExpedition(theName:string, rawData:string):void
{
	this.name = theName;
	...
	pol.forEach ( (tws:string) =>
	{
		...
		{
			var polTarget:any[] = tws.split("\t");
			for (var i:number=1;i<polTarget.length;i+=2)
			{
				var pt:PolarTarget = new PolarTarget;
...
public saveToExpedition():string
{
	var tws:number = -1;
	var exp:string = "";
...
public recalculateTarget():void
{
	...
	{
		if (pt.tws > maxTWS) maxTWS = pt.tws + 1;
	}
	);
	// allocate vectors big enough to fit
	var maxVMG:number[] = new Array<number>(maxTWS);
	var targetTWA:number[] = new Array<number>(maxTWS);
	var maxDownVMG:number[] = new Array<number>(maxTWS);
	var targetDownTWA:number[] = new Array<number>(maxTWS);
 ...
 * 
 * *************************************************************************************/
public getTargetOffWind(tws:number, twd:number, ctw:number):OffWindTarget
{
	var twsM:number = this.closestTWS(tws);	// simplify by getting the closest wind match
	var target:OffWindTarget = new OffWindTarget();
	target.tws = twsM;
	target.twd = twd;
	...
	this.myPolars.forEach ( (pt:PolarTarget ) =>
	{
		if (pt.tws == twsM)
		{
			...
			var starboardBTW:number = Angles.addAngles(Angles.substractAngles(ctw,twd),pt.twa);
			var starboardVMC:number = pt.v * Math.cos(Angles.degreeToRad(starboardBTW));
			if (starboardVMC > target.vmc)
			{
				target.vmc = starboardVMC;
				target.twa = pt.twa;
				target.v = pt.v;
				target.cog = Angles.substractAngles(twd, pt.twa);
			}

			// calculate the port tack: CTW - TWD - TWA
			var portBTW:number = Angles.substractAngles(ctw,twd,pt.twa);
			var portVMC:number = pt.v * Math.cos(Angles.degreeToRad(portBTW));
			if (portVMC > target.vmc)
			{
				target.vmc = portVMC;
				target.twa = pt.twa;
				target.v = pt.v;
				target.cog = Angles.addAngles(twd, pt.twa);
			}
...
public closestTWS(tws:number):number
{
	var diff:number = Number.MAX_VALUE;
	var closestTWS:number=0;
	
	this.myPolars.forEach ( (p:PolarTarget ) =>
	{
...
public getTarget(tws:number, twa:number):PolarTarget
{
	// first we need to locate two tws that bracket 'tws'
	var lowTWS:number = 100;
	var highTWS:number = 0;
	var p:PolarTarget;
	var lowPolar:PolarTarget;
	...
	if (isNaN(lowTWS)) lowTWS = highTWS;
	if (isNaN(highTWS) ) highTWS = lowTWS;
...
private getTargetForKnownTWS(tws:number, twa:number):PolarTarget
{
	// for each locate two polars that bracket twa
	var lowPolar:PolarTarget = new PolarTarget();
	//var highPolar:PolarTarget;
	//var pt:PolarTarget;
	...
	{
		twa = Math.abs(twa);
		if (twa > 180) twa = Math.abs(twa - 360);
		...
		this.myPolars.forEach ( (p:PolarTarget ) =>
		{
			if (p.tws == tws) // for given TWS
			{
				...
				if (p.twa < twa)  lowPolar = p;
				
				if (p.twa == twa) // exact match
					return new PolarTarget(p);	// return it.
				...
				if (p.twa > twa)  
				{
					...
					{
						var pt:PolarTarget = new PolarTarget(p); 
						pt.twa = twa;
						pt.tws = tws;
						pt.v = 0;
						return pt; 
	...
	return new PolarTarget(lowPolar);
}
...
private averagePolarsTWS(lowPolar:PolarTarget, highPolar:PolarTarget, tws:number):PolarTarget
{
	...
	if (tws == lowPolar.tws) return lowPolar;
	if (tws == highPolar.tws) return highPolar;
	
	
	var slope:number = (tws - lowPolar.tws) / (highPolar.tws - lowPolar.tws);
...
// it assumes the curve between the two to be linear
private averagePolarsTWA(lowPolar:PolarTarget, highPolar:PolarTarget, twa:number):PolarTarget
{
	// check for boundary conditions
	...
	var slope:number = (twa - lowPolar.twa) / (highPolar.twa - lowPolar.twa);
