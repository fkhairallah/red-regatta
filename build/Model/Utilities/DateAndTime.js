...
class DateAndTime {
    ...
    static parseDatetime(value, guessAMPM = true) {
        var td = value.split(" ");
        if (td.length == 1)
            return DateAndTime.parseTime(value, guessAMPM);
        // if we have 2 parts see if the first has a date component
        if (td.length == 2) {
            // parse date part
            var dateParts = td[0].split("/");
            if (dateParts.length < 2)
                return null; // unless we have 2 parts assume bad date
            // we should have a good string ==> parse time first then modify date
            var d = DateAndTime.parseTime(td[1], guessAMPM);
            ...
            if (dateParts.length > 2) { // replace year
                ...
                if (d.getUTCFullYear() < 1000)
                    d.setUTCFullYear(d.getUTCFullYear() + 2000);
            }
            return d;
        }
    ...
    static parseTime(value, guessAMPM = true) {
        value = value.toUpperCase();
        var dt = new Date(0, 0, 0);
        //var time:Object;
        var isMil = false;
        //standard time regex
        var reg = /^(1[012]|[1-9])(:[0-5]\d)?(:[0-5]\d)?(\ ?[AaPp][Mm]?)?$/;
    ...
    static parseElapsedTime(value) {
        value = value.toUpperCase();
        //standard time rege
        var reg = /^(1[012]|[1-9])?(:[0-5]\d|:[0-9])(:[0-5]\d|:[0-9])?(.\d)?/;
        var matches = reg.exec(value);
        if (!matches)
            return -1; //could not parse ==> signal error
    ...
    static elapsedTime(startTime, endTime, showSeconds = false) {
        if (startTime == null) {
    ...
    static formatAsTime(time, showSeconds = false) {
        var tString = "";
        if (time < 0)
            return "∞";
        if (time == 0)
            return "-";
        // days
        var days = Math.trunc(time / (24 * 60 * 60 * 1000));
        if (days > 99)
            return "∞"; // are you kidding me?
        if (days > 0)
            tString = days.toString() + "d ";
        // hours
        var hours = Math.trunc(time / (60 * 60 * 1000)) % 24;
        if (hours < 10)
            tString += " ";
        tString += hours.toString() + ":";
        // minutes
        var minutes = Math.trunc(time / (60 * 1000)) % 60;
        if (minutes < 10)
            tString += "0";
